...
GENRES = ["Blues", "Classic", "Country", "Disco", "Hip-hop", "Jazz", "Metal", "Pop", "Reggae", "Rock"]
...
def record_audio():
    ...
    try:
        progress_label.config(text="Recording audio...")
        recording = sd.rec(int(duration * sample_rate), samplerate=sample_rate, channels=1)
        sd.wait()  # Wait until recording is finished
        wavio.write(file_path, recording, sample_rate, sampwidth=2)
        progress_label.config(text="Audio recorded successfully.")
...
record_button = tk.Button(root, text="Record Audio", command=record_audio, font=button_font, bg=button_color, fg="white", bd=0, relief="flat", state="disabled", height=2, width=20)
